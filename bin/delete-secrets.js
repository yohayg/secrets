...
const {SecretsManagerClient, DeleteSecretCommand, ListSecretsCommand} = require("@aws-sdk/client-secrets-manager");
...
function showHelp() {
    console.log(`
        Delete
        all secrets from AWS Secrets Manager within a region, environment and namespace.
By default, you will be prompted for confirmation deleting.

!! This is destructive, be careful using it !!

Usage: delete
        -secrets --namespace my-namespace --env staging
...
function prompt(question) {
    ...
    return new Promise(function (resolve) {
        readLn.question(question, function (answer) {
            resolve(answer && answer.length && answer.trim().toLowerCase() === 'yes' || answer.trim().toLowerCase() === 'y');
        });
    });
...
async function listSecrets() {
    ...
    do {
        ...
        try {
            ...
            res = await secretsmanager.send(command);
        } catch (err) {
...
function deleteSecrets(list) {
    ...
    for (let item of list) {
        ...
        let promise = secretsmanager.deleteSecret(command);
