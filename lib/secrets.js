...
const { SecretsManagerClient, GetSecretValueCommand, CreateSecretCommand, ListSecretsCommand, DeleteSecretCommand } = require("@aws-sdk/client-secrets-manager");
...
class Secrets {
     ...
     * @param  {String|undefined} options.delimiter delimiter used in key names (default:'/')
     * @param  {String|undefined} options.env The environment or stage the secret belongs to
     * e.g.: staging/database/secret. This is important when generating secret
     * config so that only secrets for specific environments are used.
     * If not provided `process.env.NODE_ENV` is used.
     * @param  {String|Array|undefined} options.namespace An optional namespace to be prepended.
     * e.g.: my-namespace/production/database/secret
     * @param  {Boolean|undefined} options.all Ignore the environment and retrieve all secrets
     */
    constructor(options = {}) {
        ...
        this.secretsmanager = this.region;
     ...
     * @param  {String|undefined} options.version The secret version
     * @param  {String|undefined} options.stage staging label attached to the version
     * @param  {Boolean} options.raw return all the raw data from AWS instead of
     *    just the secret
    ...
    async getSecret(options = {}) {
        ...
        try {
            ...
            if (
                ...
                this.retryAttempts++;
                console.error(new Error(`Rate limit exceeded. Retry attempt ${this.retryAttempts} of ${Secrets.MAX_RETRY_ATTEMPTS}.`));
     ...
     * @param  {Object} options={}
     * @param  {String} options.id The id of the secret to retrieve
     * @param  {Number|undefined} options.maxBuffer The maxBuffer for the spawned process
     * @param  {String|undefined} options.version The secret version
     * @param  {String|undefined} options.stage staging label attached to the version
     * @param  {Boolean|undefined} options.raw return all the raw data from AWS instead of just the secret
     ...
     * will automatically append the namespace and env if they are provided
     *
     ...
     * @param  {String} options.kms secret kms
     * @param  {[Object]} options.tags secret tags
     * @param  {Object:String} options.secrets secret string
     * @param  {Buffer} options.secretsBinary secret secrets as binary
    ...
    async listSecrets() {
        ...
        do {
            ...
            try {
                ...
                res = await this.secretsmanager.send(command);
            } catch (err) {
     ...
     * `force` is true
     *
     * @param  {String} id The id of secret to delete
     * @param  {Boolean} force=false force the secret to be immediately deleted
